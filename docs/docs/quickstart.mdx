...
Install first: clone, then run the one-command bootstrap `./scripts/setup.sh` (or `.\scripts\setup.ps1` on Windows). It builds the server, prompts for the filesystem allowlist the first time, wires the skill and MCP server into pi, Cursor, and Codex, and verifies the result — then restart your agent. The server requires MCP protocol `2026-07-28`.
