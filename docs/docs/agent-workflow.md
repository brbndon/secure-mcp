...
| `suppressed` | Documented false positive |
| `accepted_risk` | Conscious residual with owner/rationale |
| `not_applicable` | Out of scope or not present |
| `deferred` | Real but postponed |
| `fixed` | Remediation revalidated with evidence |

Pass confirmed open work (`reportable` and, when intentionally postponed, `deferred`) into `secure_mcp_produce_findings`. Open work sorts before unconfirmed `needs_review` candidates. `fixed`, `suppressed`, `accepted_risk`, and `not_applicable` remain counted in `candidate_disposition_counts`, but are excluded from open risk and `remediation_priority`. Git “still present” checks stay on the host agent; the server does not run git.
