...
Two install paths: clone + `./scripts/setup.sh` ships the skill and installer; `npx -y @brdndon/secure-mcp@2` starts the published 2.0.0 bin only and does **not** install the skill. This table does not claim every client can connect — only clients that negotiate `2026-07-28` will. Grok is listed as unsupported on purpose.
...
Claude Desktop reads `claude_desktop_config.json` from its macOS application-support directory:
