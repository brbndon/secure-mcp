...
The site is a single Blume build: the marketing pages live in `pages/` (custom .astro routes served at `/`, `/help`, `/support`, `/privacy`, `/terms`) and the documentation lives under `/docs/*` (content in `docs/docs/`). Build and validate it locally before publishing through your chosen static host; this repository does not run an automatic docs workflow.
