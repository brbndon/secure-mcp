...
To skip the prompt, pass the allowlist explicitly: `SECURE_MCP_ALLOWED_ROOTS=/absolute/path/to/Code ./scripts/setup.sh`. Prefer a parent directory that contains the Swift and web checkouts you review.
...
The npm tarball contains only the compiled server and public project documents (`dist/`, README, LICENSE, NOTICE, CHANGELOG). It does not include the agent skill, installer, fixtures, source, `examples/`, or `server.json` registry metadata. Copy the skill from a checkout (or `examples/agents-md-snippet.md`) yourself if you used `npx`.
